...
- **The home rail's feature icons do not exist** — `Entrypoint 1` gives each of its four features a glyph (a camera, layers, a target, a document) and none are in this repo. `Chips` draws its iconSlot placeholder when `showLeftIcon` is left on, so four identical squares appeared that read as a rendering failure; the rail runs `showLeftIcon={false}` instead. Request the four glyphs, or accept a text-only rail.
- **No flame for the streak** — the chat header's streak count binds `accent/coral/bold` correctly but reuses the bolt, because there is no flame icon. Cosmetic, and visible.
